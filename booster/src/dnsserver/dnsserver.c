...
static dns_server_t dns_server_data = {0};
...
static int dns_socket_bind(struct udp_pcb **udp, uint32_t ip, uint16_t port) {
    ...
    if (err != ERR_OK) {
        DPRINTF("dns failed to bind to port %u: %d", port, err);
        assert(false);
    }
    return err;
}
...
static int dns_socket_sendto(struct udp_pcb **udp, const void *buf, size_t len, const ip_addr_t *dest, uint16_t port) {
    ...
    if (p == NULL) {
        DPRINTF("DNS: Failed to send message out of memory\n");
        return -ENOMEM;
    }
    ...
    if (err != ERR_OK) {
        DPRINTF("DNS: Failed to send message %d\n", err);
        return err;
    }
...
static void dns_server_process(void *arg, struct udp_pcb *upcb, struct pbuf *p, const ip_addr_t *src_addr, u16_t src_port) {
    ...
    // DPRINTF("len %d\n", msg_len);
    // DPRINTF("dns flags 0x%x\n", flags);
    // DPRINTF("dns question count 0x%x\n", question_count);
    ...
    if (((flags >> 15) & 0x1) != 0) {
        DPRINTF("Ignoring non-query\n");
        goto ignore_request;
    }

    // Check for standard query
    if (((flags >> 11) & 0xf) != 0) {
        DPRINTF("Ignoring non-standard query\n");
        goto ignore_request;
    }

    // Check question count
    if (question_count < 1) {
        DPRINTF("Invalid question count\n");
        goto ignore_request;
    }
    ...
    while(question_ptr < question_ptr_end) {
        if (*question_ptr == 0) {
            ...
            if (label_len > 63) {
                // DPRINTF("Invalid label\n");
                goto ignore_request;
            }
            // DPRINTF("%.*s", label_len, question_ptr);
    ...
    if (question_ptr - question_ptr_start > 255) {
        DPRINTF("Invalid question length\n");
        goto ignore_request;
    }
...
void dns_server_init(ip_addr_t *ip) {
    dns_server_t *d = &dns_server_data;
    if (dns_socket_new_dgram(&d->udp, d, dns_server_process) != ERR_OK) {
        DPRINTF("dns server failed to start\n");
        return;
    }
    if (dns_socket_bind(&d->udp, 0, PORT_DNS_SERVER) != ERR_OK) {
        DPRINTF("dns server failed to bind\n");
        return;
    }
    ip_addr_copy(d->ip, *ip);
    DPRINTF("dns server listening on port %d\n", PORT_DNS_SERVER);
}

...
void __not_in_flash_func(dma_irq_handler_lookup)(void)
{
    ...
    uint16_t addr_lsb = dma_hw->ch[lookup_data_rom_dma_channel].al3_read_addr_trig;
...
{
    // DPRINTF("DMA IRQ\n");
    uint32_t addr = (uint32_t)dma_hw->ch[read_addr_rom_dma_channel].al3_read_addr_trig;
...
int init_romemul(IRQInterceptionCallback requestCallback, IRQInterceptionCallback responseCallback, bool copyFlashToRAM)
{
    ...
    pio_sm_put_blocking(default_pio, smReadROM, ((unsigned long int)&__rom_in_ram_start__ >> 16));

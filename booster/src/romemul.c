...
void __not_in_flash_func(dma_irq_handler_lookup)(void) {
  ...
  // Restart the DMA
  //    dma_channel_start(lookup_data_rom_dma_channel);
}

void __not_in_flash_func(dma_irq_handler_address)(void) {
  // DPRINTF("DMA IRQ\n");
  uint32_t addr =
      (uint32_t)dma_hw->ch[read_addr_rom_dma_channel].al3_read_addr_trig;
  uint16_t value = *((uint16_t *)addr);
  DPRINTF("DMA ADDR: $%x, VALUE: $%x\n", addr, value);

  // Clear the interrupt request for the channel
  dma_hw->ints0 = 1u << read_addr_rom_dma_channel;

  // Restart the DMA
  //    dma_channel_start(read_addr_rom_dma_channel);
}
...
static int init_rom_emulator(PIO pio, IRQInterceptionCallback requestCallback,
  ...
  dma_channel_config cdmaLookup =
      dma_channel_get_default_config(lookup_data_rom_dma_channel);
  ...
  dma_channel_configure(lookup_data_rom_dma_channel, &cdmaLookup,
  ...
  dma_channel_config cdma =
      dma_channel_get_default_config(read_addr_rom_dma_channel);
  ...
  dma_channel_configure(
      read_addr_rom_dma_channel, &cdma,
      &dma_hw->ch[lookup_data_rom_dma_channel].al3_read_addr_trig,
...
int init_romemul(IRQInterceptionCallback requestCallback,
  ...
  if (copyFlashToRAM) {
    ...
    COPY_FIRMWARE_TO_RAM(src_addr, ROM_SIZE_WORDS * ROM_BANKS * 2);
  }
  ...
  int smReadROM =
      init_rom_emulator(default_pio, requestCallback, responseCallback);
  if (smReadROM < 0) {
  ...
  // 15 bits from the rp2040 memory address, so shift right 17 bits to get the
  // 15 bits In the PIO program, the address is shifted left 1 bit to make room
  ...
  // the rp2040 and the FLASH memory used are defined in the file
  // memmap_romemul.ld
  // Please do not modify these values, because they are carefully selected to
  // avoid conflicts and be performant.

  pio_sm_put_blocking(default_pio, smReadROM,
                      ((unsigned long int)&__rom_in_ram_start__ >> 16));
